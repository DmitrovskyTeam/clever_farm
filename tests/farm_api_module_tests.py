...
class BasicModuleTest(unittest.TestCase):
    ...
    def test_temperature(self):
        ...
        for i in range(1, 5):
            response = str(self.api_module.get_air_temp_hum(sensor_id=i))
    ...
    def test_ground_sensor(self):
        ...
        for i in range(1, 7):
            response = str(self.api_module.get_ground_hum(sensor_id=i))
            self.assertIn('id', response)
            self.assertIn('humidity', response)
    ...
    def test_hydration_system(self):
        ...
        self.assertDictEqual({'code': 404, 'message': 'State value not found'},
                             session.patch(url='https://dt.miet.ru/ppo_it/api/total_hum',
                                           params={
                                               'state': str(-1)
                                           }).json())
        self.assertDictEqual({'code': 404, 'message': 'State value not found'},
                             session.patch(url='https://dt.miet.ru/ppo_it/api/total_hum').json())
    ...
    def test_window_system(self):
        ...
        session = requests.Session()
        self.assertDictEqual({'code': 404, 'message': 'State value not found'},
                             session.patch(url='https://dt.miet.ru/ppo_it/api/fork_drive',
                                           params={
                                               'state': str(-1)
                                           }).json())
        self.assertDictEqual({'code': 404, 'message': 'State value not found'},
                             session.patch(url='https://dt.miet.ru/ppo_it/api/fork_drive').json())
