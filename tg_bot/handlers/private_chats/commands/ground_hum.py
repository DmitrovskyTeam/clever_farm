...
async def ground_sensors_request(message: types.Message):
    ...
    for i in range(0, len(sensor_values)):
        text.append(
            f"<b>{sensor_values[i].get('timestamp').split('.')[0]}:   </b>{(sensor_values[i].get('sensor1') + sensor_values[i].get('sensor2') + sensor_values[i].get('sensor3') + sensor_values[i].get('sensor4') + sensor_values[i].get('sensor5') + sensor_values[i].get('sensor6')) / 6}")
